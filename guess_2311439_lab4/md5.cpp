...
void My_MD5Hash(string inputs[4], bit32 states[4][4]) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        assert(messageLengths[i] == messageLengths[0]); // 确保长度一致
    }
    ...
    for (int i = 0; i < 4; ++i) {
        for (int j = 0; j < 4; ++j) {
            states[i][j] = results[j][i];
            // 字节序调整
            states[i][j] = ((states[i][j] & 0xff) << 24) |
                           ((states[i][j] & 0xff00) << 8) |
                           ((states[i][j] & 0xff0000) >> 8) |
                           ((states[i][j] & 0xff000000) >> 24);
        }
    }
